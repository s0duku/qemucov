...
#ifdef QEMUCOV_PATCH
void HELPER(qemucov_bb_exec_record)(target_ulong cur_loc) {
    target_ulong bit_offset;
    target_ulong byte_idx;
    qemucov_image_info * image_info;
    // first get cur_loc image
    image_info = qemucov_image_mapping_list;
    while(image_info) {
        //printf("cur loc:%p, image: %s, start %p, end: %p\n",cur_loc,image_info->module_path,image_info->start,image_info->end);
        if(image_info->start <= cur_loc && cur_loc < image_info->end){
            byte_idx = cur_loc-image_info->start;
            // printf("basic block %s, 0x%x\n",image_info->module_path,byte_idx);
            // get bit offset
            bit_offset = byte_idx % 8;
            // bitmap one byte for 8 address
            byte_idx = byte_idx / 8;
            // set bitmap
            qemucov_bb_exec_map[image_info->module_id]->bitmap[byte_idx] = (1 << bit_offset) | qemucov_bb_exec_map[image_info->module_id]->bitmap[byte_idx];
            break;
        }
...
static void qemucov_gen_trace(target_ulong cur_loc) {
    TCGv cur_loc_v = tcg_const_tl(cur_loc);
    // helper function maybe not change pc value, more like side effect
    gen_helper_qemucov_bb_exec_record(cur_loc_v);
                           ...
                           int *max_insns, int64_t *ti)
{
    ...
    #ifdef QEMUCOV_PATCH
    // setup global infomation before TCG JIT start
    qemucov_setup();
    // use bitmap avoid multi thread this problem
    qemucov_gen_trace(pc);
    #endif
